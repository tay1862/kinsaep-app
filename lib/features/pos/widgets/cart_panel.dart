...
class CartPanel extends ConsumerWidget {
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  Text(l10n.cart, style: const TextStyle(
                  ...
                  if (cartItems.isNotEmpty)
                    TextButton.icon(
                      ...
                      style: TextButton.styleFrom(foregroundColor: KinsaepTheme.error),
                    ),
                ],
              ),
            ),
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(l10n.emptyCart, style: const TextStyle(
                            color: KinsaepTheme.textSecondary, fontSize: 16,
                          )),
                        ],
                      ),
                    )
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      CurrencyUtil.format(item.unitPrice, currency),
                                      style: const TextStyle(
                                        color: KinsaepTheme.textSecondary, fontSize: 13,
                                      ),
                                    ),
                                  ],
                                ),
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    _QtyButton(
                                      ...
                                      color: item.quantity > 1 ? KinsaepTheme.textSecondary : KinsaepTheme.error,
                              ...
                              SizedBox(
                                width: 80,
                                child: Text(
                                  CurrencyUtil.format(item.totalPrice, currency),
            ...
            if (cartItems.isNotEmpty)
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: Colors.black.withOpacity(0.06), // Corrected withValues to withOpacity
                      blurRadius: 16,
                ...
                child: SafeArea(
                  child: Column(
                    children: [
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: Row(
                            children: [
                              _DiscountPreset(
                                label: 'VIP 10%',
                                isActive: ref.watch(discountPercentProvider) == 10,
                                onTap: () {
                                  final current = ref.read(discountPercentProvider);
                                  ref.read(discountPercentProvider.notifier).state = current == 10 ? 0 : 10;
                              ...
                              _DiscountPreset(
                                label: 'Happy Hour 15%',
                                isActive: ref.watch(discountPercentProvider) == 15,
                                onTap: () {
                                  final current = ref.read(discountPercentProvider);
                                  ref.read(discountPercentProvider.notifier).state = current == 15 ? 0 : 15;
                                  ref.read(discountAmountProvider.notifier).state = 0;
                                },
                              ),
                              const SizedBox(width: 8),
                              _DiscountPreset(
                                label: 'Staff 50%',
                                isActive: ref.watch(discountPercentProvider) == 50,
                                onTap: () {
                                  final current = ref.read(discountPercentProvider);
                                  ref.read(discountPercentProvider.notifier).state = current == 50 ? 0 : 50;
                                  ref.read(discountAmountProvider.notifier).state = 0;
                                },
                              ),
                              const SizedBox(width: 8),
                              _DiscountPreset(
                                label: 'Custom %',
                                isActive: false,
                                onTap: () => _showCustomDiscountDialog(context, ref, l10n, subtotal),
                              ),
                            ],
                          ),
                        ),
                      ...
                      // Subtotal
                      _SummaryRow(label: l10n.subtotal, value: CurrencyUtil.format(subtotal, currency)),
                      if (discountTotal > 0) ...[
                        const SizedBox(height: 4),
                        _SummaryRow(
                          label: l10n.discount,
                          value: '-${CurrencyUtil.format(discountTotal, currency)}',
                      ...
                      Row(
                        children: [
                          Expanded(
                            ...
                            child: OutlinedButton(
                              ...
                              style: OutlinedButton.styleFrom(
                                ...
                                side: const BorderSide(color: KinsaepTheme.error),
                          ...
                          Expanded(
                            flex: 1,
                            child: OutlinedButton(
                              onPressed: () => _saveTicket(context, ref, l10n),
                              style: OutlinedButton.styleFrom(
                                padding: const EdgeInsets.symmetric(vertical: 16),
                                foregroundColor: KinsaepTheme.primary,
                                side: const BorderSide(color: KinsaepTheme.primary),
                                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                              ),
                          ...
                          Expanded(
                            ...
                            child: ElevatedButton(
                              onPressed: cartItems.isEmpty ? null : () {
                                Navigator.pop(context); // Close cart panel
                              ...
                              child: Text(l10n.charge, style: const TextStyle(
  ...
  void _saveTicket(BuildContext context, WidgetRef ref, AppLocalizations l10n) {
    ...
    final nameController = TextEditingController(text: ref.read(activeTicketNameProvider) ?? '');
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: Text(l10n.cancel, style: const TextStyle(color: KinsaepTheme.textSecondary)),
          ),
          ElevatedButton(
            onPressed: () async {
              ...
              if (activeTicketId == null) {
                ...
                saleUpdates['receiptNumber'] = 'TICKET-${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}';
                await db.insertSale(saleUpdates, saleItemsList);
              } else {
                await db.updateTicketToSale(saleId, saleUpdates, saleItemsList);
              }
  ...
  static void _showCustomDiscountDialog(BuildContext context, WidgetRef ref, AppLocalizations l10n, double subtotal) {
    final controller = TextEditingController();
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Custom Discount', style: TextStyle(fontWeight: FontWeight.bold)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
...
class _SummaryRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(label, style: const TextStyle(color: KinsaepTheme.textSecondary, fontSize: 14)),
...
class _DiscountPreset extends StatelessWidget {
  ...
  const _DiscountPreset({required this.label, required this.isActive, required this.onTap});
