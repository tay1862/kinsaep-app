...
class TicketsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Open Tickets', style: TextStyle(fontWeight: FontWeight.w800)),
          ...
          if (tickets.isEmpty) {
            return Center(
              child: Column(
                ...
                children: [
                  const Icon(Icons.receipt_long_rounded, size: 64, color: KinsaepTheme.border),
                  const SizedBox(height: 16),
                  Text('No open tickets', style: TextStyle(
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.table_restaurant_rounded, color: KinsaepTheme.primary),
                        ),
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(name, style: const TextStyle(
                                fontSize: 16, fontWeight: FontWeight.w700, color: KinsaepTheme.textPrimary,
                              )),
                              const SizedBox(height: 4),
                              Text('Opened at $formattedDate', style: const TextStyle(
                          ...
                          children: [
                            Text(
                              ...
                              style: const TextStyle(
                                fontSize: 16, fontWeight: FontWeight.w800, color: KinsaepTheme.accent,
                              ),
                            ),
                            const SizedBox(height: 4),
                            IconButton(
                              onPressed: () => _deleteTicket(context, ref, ticket['id'] as String),
                              icon: const Icon(Icons.delete_outline_rounded, color: KinsaepTheme.error, size: 20),
  ...
  Future<void> _resumeTicket(BuildContext context, WidgetRef ref, Map<String, dynamic> ticket) async {
    ...
    ref.read(discountAmountProvider.notifier).state = (ticket['discountAmount'] as num).toDouble();
    ref.read(discountPercentProvider.notifier).state = (ticket['discountPercent'] as num).toDouble();
    
    ref.read(activeTicketIdProvider.notifier).state = saleId;
    ref.read(activeTicketNameProvider.notifier).state = ticket['ticketName'] as String?;
    
    if (context.mounted) Navigator.pop(context);
  }

  Future<void> _deleteTicket(BuildContext context, WidgetRef ref, String saleId) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Delete Ticket'),
        content: const Text('Are you sure you want to delete this open ticket?'),
        actions: [
